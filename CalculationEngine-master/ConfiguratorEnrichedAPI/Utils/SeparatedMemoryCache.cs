namespace ConfiguratorEnrichedAPI.Utils
{
    ///<summary>
    ///Implements a MemoryCache where each class (<typeparamref name="THostClass"/>) does not clash with the cache keys of other classes
    ///</summary>
    public class SeparatedMemoryCache<THostClass>(IMemoryCache cache) : IMemoryCache where THostClass : class
    {
        private static readonly string typeName = typeof(THostClass).FullName ?? throw new Exception("Missing target type");

        public ICacheEntry CreateEntry(object key) =>
            cache.CreateEntry(GetKey(key));

        public void Dispose() =>
            GC.SuppressFinalize(this);

        public T? GetOrCreate<T>(object key, Func<ICacheEntry, T> factory) =>
            cache.GetOrCreate(GetKey(key), factory);

        public void Remove(object key) =>
            cache.Remove(GetKey(key));

        public bool TryGetValue(object key, out object? value) =>
            cache.TryGetValue(GetKey(key), out value);
...
/* In case it is needed to enumerate the cache entries:
using System;
